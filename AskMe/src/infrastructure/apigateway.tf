...
resource "aws_apigatewayv2_integration" "lambda_whatsapp_integration_" {
  api_id           = aws_apigatewayv2_api.bedrock_api.id
  integration_type = "AWS_PROXY"
  integration_uri  = aws_lambda_function.lambda_whatsapp.invoke_arn
}

resource "aws_apigatewayv2_route" "lambda_whatsapp_route_post" {
  api_id    = aws_apigatewayv2_api.bedrock_api.id
  route_key = "POST /bedrock" #defines the routes and methods that will trigger the lambda
  target    = "integrations/${aws_apigatewayv2_integration.lambda_whatsapp_integration_.id}"
}

resource "aws_apigatewayv2_route" "lambda_whatsapp_route_get" {
  api_id    = aws_apigatewayv2_api.bedrock_api.id
  route_key = "GET /bedrock" #defines the routes and methods that will trigger the lambda
  target    = "integrations/${aws_apigatewayv2_integration.lambda_whatsapp_integration_.id}"
}

resource "aws_apigatewayv2_route" "lambda_whatsapp_route_put" {
  api_id    = aws_apigatewayv2_api.bedrock_api.id
  route_key = "PUT /bedrock" #defines the routes and methods that will trigger the lambda
  target    = "integrations/${aws_apigatewayv2_integration.lambda_whatsapp_integration_.id}"
}

resource "aws_apigatewayv2_route" "lambda_whatsapp_route_delete" {
  api_id    = aws_apigatewayv2_api.bedrock_api.id
